...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            ...
            if precond_s_nodes.issubset(s_level):
                for preceding_s_node in s_level:
    ...
    def add_literal_level(self, level):
        ...
        # get all the action
        action = self.a_levels[level -1]
        # create a set for the level-1 state
        s_level = set()
...
# precond_neg
        ...
        for effect_a in node_a1.action.effect_rem:
            for precond_b in node_a2.action.precond_pos:
                if effect_a == precond_b: return True
        ...
        for effect_b in node_a2.action.effect_rem:
            for precond_a in node_a1.action.precond_pos:
                if effect_b == precond_a: return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        for precond_a in node_a1.parents:
            for precond_b in node_a2.parents:
                if precond_a.is_mutex(precond_b): return True
